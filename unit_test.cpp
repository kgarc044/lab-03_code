...
TEST(Test, MultFunctions) {
	Op* op1 = new Op(6.0);
	Op* op2 = new Op(6.0);
	Add* addObj = new Add(op1, op2);
	Op* op3 = new Op(2.0);
	Sub* subObj = new Sub(addObj, op3);
...
TEST(Test, RandClass) {
	Rand* rand = new Rand();
	RandMock* rand2 = new RandMock();
	EXPECT_EQ(rand->evaluate(), rand2->evaluate());
	EXPECT_EQ(rand->stringify(), rand2->stringify());
